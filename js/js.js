...
function checkEmail(){
	...
	if(index==-1){
		email_prompt.innerHTML="<img src='../images/error.png'>输入的邮箱格式中应包含@符号";
		return false;
	}
	if(email.indexOf(".",index)==-1){
		email_prompt.innerHTML="<img src='../images/error.png'>输入的邮箱格式中应包含.符号且在@符号后面"
		return false;
	}
...
function checkAll() {
	if(checkUser() && checkPwd() && checkRepwd() && checkEmail() && checkMobile()) {
		return true;		
	} else {
		return false;
	}
